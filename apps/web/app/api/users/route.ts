...
const entrySchema = z.object({
  ...
  activityTypes: z.array(z.string().trim().min(1).max(80)).max(20).default([]),
  contraceptionMethod: z.enum(["pill", "ring", "patch", "iud", "implant", "injection", "condom", "other"]).optional(),
  contraceptionStatus: z.enum(["on_time", "missed", "changed", "stopped"]).optional(),
  pregnancyTest: z.enum(["negative", "positive", "unclear"]).optional(),
  ovulationTest: z.enum(["negative", "positive", "unclear"]).optional(),
  sexualActivity: z.enum(["none", "protected", "unprotected"]).optional(),
  sexualComfort: z.enum(["comfortable", "dryness", "pain", "bleeding"]).optional(),
