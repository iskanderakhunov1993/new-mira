...
export default function InsightsPage() {
  ...
  return <main className="insights-page"><div className="insights-shell">
    ...
    {ready && <>
      ...
      <section className="patterns-list"><div className="patterns-heading"><div><small>На основе ваших отметок</small><h2>Мои закономерности</h2><p>Когда симптомы повторяются в похожие дни цикла.</p></div><span>{personalization.patterns.length}</span></div>{personalization.patterns.length ? personalization.patterns.slice(0, 3).map((pattern) => <SymptomPatternCard pattern={pattern} key={pattern.name} />) : <div className="insights-empty"><Lightbulb /><h2>Повторений пока не видно</h2><p>Симптом появится здесь, когда повторится хотя бы в половине из трёх или более циклов.</p></div>}</section>
