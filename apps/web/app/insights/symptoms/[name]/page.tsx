...
export default function SymptomDetailPage() {
  ...
  if (!pattern) return <main className="cycle-subpage"><div className="analytics-empty"><h2>Пока недостаточно данных</h2><p>Нужны минимум три завершённых цикла и повторение симптома хотя бы в половине из них.</p><Link href="/insights">Вернуться к инсайтам</Link></div></main>;
  return <main className="cycle-subpage"><div className="cycle-subpage-shell symptom-detail"><header className="cycle-subpage-top"><Link href="/insights" aria-label="Назад"><ChevronLeft /></Link><h1>Симптом</h1><i /></header><section className="symptom-detail-hero"><Sparkles /><small>Личный паттерн · {Math.round(pattern.recurrenceRate * 100)}%</small><h2>{pattern.name}</h2><p>Повторялся в {pattern.matchedCycles} из {pattern.evaluatedCycles} циклов, обычно на {pattern.dayRange.min}–{pattern.dayRange.max}-й день.</p></section><section className="symptom-history"><header><h2>Отметки по циклам</h2><Info /></header>{pattern.cycles.map(({ cycle, days, averageIntensity }) => <article key={cycle.start}><div><strong>{formatCycleDate(cycle.start)}</strong><span>{days.map((day) => `${day}-й день`).join(", ")}</span></div><b>{averageIntensity ? `Интенсивность ${averageIntensity.toFixed(1)} / 3` : "Без интенсивности"}</b></article>)}</section><section className="cycle-detail-facts"><Info /><div><h2>Как использовать наблюдение</h2><p>Это повторение по вашим записям, а не прогноз или причина симптома. Если симптом усиливается или беспокоит, обсудите историю с врачом.</p></div></section></div></main>;
}
