...
export default function ProfilePage() {
  ...
  return <main className="profile-page"><div className="profile-shell">
    ...
    <section className="profile-section profile-list"><header><div><h2>Данные и приватность</h2><p>Все данные этого прототипа хранятся только в браузере.</p></div></header><Link href="/analytics/report"><i><FileHeart /></i><span><strong>Отчёт для врача</strong><small>Настроить состав и сохранить PDF</small></span><ChevronRight /></Link><button onClick={exportData}><i><Download /></i><span><strong>Скачать все данные</strong><small>Файл JSON · {entryCount} отметок</small></span><ChevronRight /></button><div className="profile-storage"><Database /><p><strong>Локальное хранение</strong><span>Данные не отправляются на сервер и доступны только в этом браузере.</span></p></div></section>
