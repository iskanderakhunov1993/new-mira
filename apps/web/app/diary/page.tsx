...
import { BarChart3, CalendarDays, Check, ChevronLeft, ChevronRight, CircleAlert, Droplet, FileText, GlassWater, Minus, MoonStar, Pill, Plus, Save, Scale, Search, Smile, Thermometer, X } from "lucide-react";
...
const moodOptions: { value: CycleEntry["mood"]; label: string }[] = [
  { value: "low", label: "Тяжело" },
  { value: "calm", label: "Спокойно" },
  { value: "good", label: "Хорошо" },
];

const emptyGroupOptions = ["Всё в порядке", "Без изменений", "Выделений нет", "Секса не было", "Без тренировки", "Ничего не принимала"];
const exclusiveSets = [
  ["Секса не было", "Секс с защитой", "Секс без защиты"],
  ["Без тренировки", "Ходьба", "Растяжка", "Йога", "Тренировка", "Бег", "Плавание", "Велосипед", "Аэробика или танцы", "Командный спорт", "Высокая нагрузка"],
...
function DiaryContent() {
  ...
  const quickFeelings = [
    { label: "Спазмы", selected: symptoms.includes("Спазмы"), action: () => toggleSymptom("Спазмы") },
    { label: "Чувствительная грудь", selected: symptoms.includes("Чувствительная грудь"), action: () => toggleSymptom("Чувствительная грудь") },
    { label: "Усталость", selected: symptoms.includes("Усталость"), action: () => toggleSymptom("Усталость") },
    { label: "Вздутие", selected: symptoms.includes("Вздутие"), action: () => toggleSymptom("Вздутие") },
  ];
  ...
  return (
    <main className={`diary-page diary-tracker-page ${symptomsMode ? "symptom-entry-page" : ""}`}>
      <div className="diary-shell">
        ...
        <section className="feeling-today-card"><h2>Как вы себя чувствуете сегодня?</h2><p>Частые отметки — можно выбрать несколько</p><div>{quickFeelings.map((item) => <button className={item.selected ? "selected" : ""} type="button" onClick={item.action} key={item.label}><span><SymptomIcon label={item.label} /></span><strong>{item.label}</strong></button>)}</div></section>
        {hasSelection && <section className="daily-summary has-selection"><div className="summary-heading"><div><small>Выбрано сегодня</small><h2>Ваши отметки</h2></div><span>{[period, pain > 0, ...symptoms, ...medicationIntakes, mood, notes.trim()].filter(Boolean).length}</span></div><div className="summary-chips">{selectedMood && <span className="summary-chip mood"><i><SymptomIcon label={selectedMood.label} /></i>{selectedMood.label}</span>}{period && <span className="summary-chip period"><Droplet />{period === "spotting" ? "Мажущие" : period === "light" ? "Слабые" : period === "medium" ? "Средние" : "Обильные"}</span>}{medicationIntakes.length > 0 && <span className="summary-chip medication"><Pill />{medicationIntakes.length} приём</span>}{symptoms.slice(0, 4).map((item) => <span className="summary-chip symptom" key={item}><SymptomIcon label={item} />{item}</span>)}{symptoms.length > 4 && <span className="summary-chip more">+{symptoms.length - 4}</span>}</div></section>}
        <div className="state-category-heading symptom-category-heading"><div><h2>Категории</h2><small>{trackingOptionCount} вариантов отметок</small></div><button type="button" onClick={() => setEditingCategories((value) => !value)}>{editingCategories ? "Готово" : "Настроить"}</button></div>
        {editingCategories && <div className="category-editor">{symptomGroups.map((group) => <button className={hiddenGroups.includes(group.id) ? "" : "selected"} type="button" onClick={() => setHiddenGroups((current) => current.includes(group.id) ? current.filter((id) => id !== group.id) : [...current, group.id])} key={group.id}><Check />{group.title}</button>)}</div>}
        {!symptomsMode && (matches("месячные") || matches("боль") || matches("интенсивность")) && <section className="state-card state-period period-p0-card" id="diary-period">
          ...
          <h3>Интенсивность выделений</h3><div className="state-chips flow-chips"><button className={period === undefined ? "selected" : ""} type="button" onClick={() => setPeriod(undefined)}><i><X /></i>Нет</button><button className={period === "spotting" ? "selected" : ""} type="button" onClick={() => setPeriod("spotting")}><i><Droplet /></i>Мажущие</button><button className={period === "light" ? "selected" : ""} type="button" onClick={() => setPeriod("light")}><i><Droplet /></i>Слабые</button><button className={period === "medium" ? "selected" : ""} type="button" onClick={() => setPeriod("medium")}><i><Droplet /></i>Средние</button><button className={period === "heavy" ? "selected" : ""} type="button" onClick={() => setPeriod("heavy")}><i><Droplet /></i>Обильные</button></div>
