  ...
  return (
    <main className={`diary-page ${symptomsMode ? "symptom-entry-page" : ""}`}>
      <div className="diary-shell">
        ...
        {symptomsMode ? <section className="feeling-today-card"><h2>Как вы себя чувствуете сегодня?</h2><p>Недавние и частые отметки</p><div>{quickFeelings.map((item) => <button className={item.selected ? "selected" : ""} type="button" onClick={item.action} key={item.label}><span><SymptomIcon label={item.label} /></span><strong>{item.label}</strong></button>)}</div></section> : <section className={`daily-summary ${hasSelection ? "has-selection" : ""}`}><div className="summary-heading"><div><small>Выбрано сегодня</small><h2>{hasSelection ? "Ваше состояние" : "Пока ничего не отмечено"}</h2></div><span>{[period, pain > 0, ...symptoms, ...medicationIntakes, mood, notes.trim()].filter(Boolean).length}</span></div>{hasSelection && <div className="summary-chips">{selectedMood && <span className="summary-chip mood"><i><SymptomIcon label={selectedMood.label} /></i>{selectedMood.label}</span>}{period && <span className="summary-chip period"><Droplet />{period === "spotting" ? "Мажущие" : period === "light" ? "Слабые" : period === "medium" ? "Средние" : "Обильные"}</span>}{medicationIntakes.length > 0 && <span className="summary-chip medication"><Pill />{medicationIntakes.length} приём</span>}{symptoms.slice(0, 3).map((item) => <span className="summary-chip symptom" key={item}><SymptomIcon label={item} />{item}</span>)}{symptoms.length > 3 && <span className="summary-chip more">+{symptoms.length - 3}</span>}</div>}</section>}
        <div className={`state-category-heading ${symptomsMode ? "symptom-category-heading" : ""}`}><div><h2>Категории</h2>{symptomsMode && <small>{trackingOptionCount} вариантов отметок</small>}</div>{symptomsMode ? <button type="button" onClick={() => setEditingCategories((value) => !value)}>{editingCategories ? "Готово" : "Редактировать"}</button> : <span>Можно выбрать несколько</span>}</div>
