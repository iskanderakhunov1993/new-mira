...
- [Карта проекта для разработки](docs/PROJECT_MAP.md) — источник истины, архитектура, маршруты, данные, авторизация, команды и release checklist.
- [Блок «Полезное сегодня»](TODAY_CARDS.md) — логика четырёх карточек, условия показа, переходы, safety-ограничения и локальный preview-режим.
- [Спецификация расчётов](CALCULATION_SPEC.md) — правила расчёта цикла и прогнозов.
- [Правила безопасности](SAFETY_RULES.md) — сценарии боли, задержки и обильных месячных.
- [QA-матрица](QA_MATRIX.md) и [production checklist](PRODUCTION_CHECKLIST.md) — проверка основных flows перед релизом.
...
- `refactor/короткое-название` — улучшение кода без изменения поведения;
- `chore/короткое-название` — зависимости, инструменты и обслуживание.
...
Сообщения пишем в формате `тип: короткое описание` на английском, например:

```text
feat: add cycle summary to today
fix: prevent duplicate period entries
docs: clarify release workflow
```

Один коммит должен содержать одно логическое изменение. Название ветки и номер версии в сообщение коммита добавлять не нужно.
...
Mira использует [семантическое версионирование](https://semver.org/lang/ru/):

- `PATCH`, например `v1.3.1` — совместимое исправление;
- `MINOR`, например `v1.4.0` — новая совместимая возможность;
- `MAJOR`, например `v2.0.0` — несовместимое изменение продукта или API.

Текущая версия хранится в `apps/web/package.json`. Production-релиз отмечается Git-тегом только в полном формате `vMAJOR.MINOR.PATCH`.

Простой выпуск версии:

1. Слить проверенные изменения в `main`.
2. Обновить версию в `apps/web/package.json` и `package-lock.json`.
3. Создать и отправить тег, например `v1.4.0`.
4. Кратко описать пользовательские изменения в GitHub Release.

Не создавайте ветки `v1.4`, `release-1.4` и похожие. Версии — это теги, работа ведётся во временных `feature/*` и `fix/*`.
